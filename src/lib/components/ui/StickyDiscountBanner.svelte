...
{#if visible}
	<div class="sticky-discount-banner fixed inset-x-0 top-0 z-50 pt-[env(safe-area-inset-top)]">
		<div
			class="flex w-full max-w-full items-center justify-center gap-3 sm:gap-4 rounded-b-[15px] bg-[#1a1a1a] py-3 pl-[max(1rem,env(safe-area-inset-left))] pr-[max(1rem,env(safe-area-inset-right))] text-white cursor-pointer select-none"
			role="button"
			tabindex="0"
			aria-label="Desconto exclusivo para este plano. Clique para ir à oferta."
			onclick={scrollToOffer}
			onkeydown={onKeydown}
		>
			<div class="flex flex-col gap-0.5 items-center sm:items-start text-center sm:text-left">
				<span class="text-[11px] font-medium text-[#9e9e9e]">Desconto exclusivo para este plano</span>
